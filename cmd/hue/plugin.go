...
func (a *app) pluginStatus(args []string) error {
	fs := flag.NewFlagSet("hue plugin status", flag.ContinueOnError)
	...
	fmt.Println("Debug logging: ", onOff(cfg.PluginDebug), "(hue plugin debug on|off)")
	return nil
}
...
func (a *app) pluginDebug(args []string) error {
	...
	fs := flag.NewFlagSet("hue plugin debug "+args[0], flag.ContinueOnError)
