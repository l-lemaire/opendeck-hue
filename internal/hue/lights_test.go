...
func pairedClient(t *testing.T, fb *huetest.Bridge) *Client {
...
func TestToggleLight(t *testing.T) {
	...
	nowOn, err := c.ToggleLight(ctx, huetest.LightKitchen)
	if err != nil || nowOn {
	...
	if len(fb.Puts()) != 2 || fb.Puts()[0] != "light/"+huetest.LightKitchen {
...
func TestSetLightBrightness(t *testing.T) {
	...
	if err := c.SetLight(ctx, huetest.LightDesk, Update{On: Bool(true), Brightness: Float(25)}); err != nil {
		t.Fatal(err)
	}
	l, _ := c.Light(ctx, huetest.LightDesk)
	...
	err := c.SetLight(ctx, huetest.LightPlug, Update{Brightness: Float(50)})
...
func TestGroups(t *testing.T) {
	...
	if kitchen.Kind != "room" || !kitchen.On || kitchen.Members != 1 || kitchen.GroupedLightID != huetest.GroupedKitchen {
...
func TestDryRunNeverWrites(t *testing.T) {
	...
	if err := c.SetGroup(ctx, Group{Name: "K", Kind: "room", GroupedLightID: huetest.GroupedKitchen}, Update{Brightness: Float(10)}); err != nil {
		t.Fatal(err)
	}
	if len(fb.Puts()) != 0 {
	...
	text := out.String()
	for _, want := range []string{"would send PUT", "/clip/v2/resource/light/" + huetest.LightKitchen, `"on": false`, "grouped_light/" + huetest.GroupedKitchen, `"brightness": 10`} {
		if !strings.Contains(text, want) {
			t.Errorf("dry-run output missing %q:\n%s", want, text)
		}
	}
	// Reads still went through.
	if l, _ := c.Light(ctx, huetest.LightKitchen); !l.IsOn() {
